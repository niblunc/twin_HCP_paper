...
%function [p_uncorrected,p_corrected] = nets_glm(netmats,des,con,view,varargin,exblocks); 
function [p_uncorrected,p_corrected] = nets_glm(netmats,des,con,view,nperms,exblocks); 
%function [p_uncorrected,p_corrected] = nets_glm(netmats,des,con,view,varargin); 
%exblocks='/projects/niblab/data/HCP_PTN820/node_timeseries/3T_HCP820_MSMAll_d25_ts2/hdiscord/PTN.grp'
%disp(exblocks)
%nperms=5000;
%if nargin==6
%  nperms=varargin{1};
...
%we changed this to include exchangability blocks
system(sprintf('randomise -i %s -o %s -d %s -t %s -e %s -x --uncorrp -n %d',fname,fname,des,con,exblocks,nperms));
