...
%addpath(sprintf('%su/matlab',getenv('FSLDIR')))    %commenteded out - may need later. you don't need to edit this if FSL is setup already


%%% setup the names of the directories containing your group-ICA and dualreg outputs
group_maps='/projects/niblab/data/HCP_PTN820/groupICA/groupICA_3T_HCP820_MSMAll_d25.ica';     % spatial maps 4D NIFTI file, e.g. from group-ICA
...
ts_dir='/projects/niblab/data/HCP_PTN820/node_timeseries/3T_HCP820_MSMAll_d25_ts2/hdiscord';                           % dual regression output directory, containing all subjects' timeseries
...
%ts_spectra=nets_spectra(ts);   %shows mean timeseries, do not need. have a look at mean timeseries spectra
...
%changed ts.DD to ts -JRG 06/09/17
pic=nets_hierarchy(Znet1,Znet2,ts.DD,group_maps); 
...
% added t inside bracket.
[p_uncorrected,p_corrected,t]=nets_glm(netmats1,'/projects/niblab/data/HCP_PTN820/node_timeseries/3T_HCP820_MSMAll_d25_ts2/hdiscord/PTN_hgl.mat','/projects/niblab/data/HCP_PTN820/node_timeseries/3T_HCP820_MSMAll_d25_ts2/hdiscord/PTN_hgl.con',0,10000, '/projects/niblab/data/HCP_PTN820/node_timeseries/3T_HCP820_MSMAll_d25_ts2/hdiscord/PTN_hgl.grp');  % returns matrices of 1-p
